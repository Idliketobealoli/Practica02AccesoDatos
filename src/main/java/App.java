...
public class App {
    public static void main(String[] args) {
        ...
        ProcessData pd = new ProcessData(args[0]); // args[0]
        CreateHTML html = new CreateHTML(pd.desiredCity, args[1]); // args[1] ---- mi carpeta de pruebas: C:\Users\User\Documents
        Result result = new Result(pd.desiredCity);
        try {
            Jaxb jaxb = Jaxb.getInstance();
            jaxb.saveIntoDB(result);

            MarkDownGenerator mdg = new MarkDownGenerator(args[0]); // args[0]
